...
const chapterGroups = [...modulesSection.matchAll(/\bchapter:\s*"([^"]+)"/g)].map((match) => match[1]);
const tierValues = [...modulesSection.matchAll(/\btier:\s*"([^"]+)"/g)].map((match) => match[1]);
...
assert(chapterGroups.length === new Set(chapterGroups).size, `Catalog chapter entries must be unique: ${JSON.stringify(chapterGroups)}`);
