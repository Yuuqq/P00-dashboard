...
const DASHBOARD_SESSION_STORAGE_KEY = "pm_metrics_session_P00-dashboard";
const DASHBOARD_VOLATILE_SESSION_PREFIX = "P00-dashboard-volatile-";
const VISIBLE_TOAST_SELECTOR = "#toastContainer > [role='status'], #toastContainer > [role='alert']";
const VISIBLE_ALERT_TEXT_SELECTOR = "#toastContainer [role='alert'] span[style*='flex: 1']";
const PROGRESS_KEY = "p00_mission_progress";
const THEME_STORAGE_KEY = "journalism_toolbox_theme";
const EXPORTED_DATA_TOAST_TEXT = "学习数据已导出";
const THEME_PREFERENCE_WARNING_TEXT = "主题偏好未写入浏览器存储，刷新后将恢复系统外观。";
const MISSION_RESET_SUCCESS_TEXT = "任务进度已重置";
const POPUP_BLOCKED_WARNING_TEXT = "浏览器拦截了新标签页，请允许弹窗后重试";
const IMPORT_CANCEL_INFO_TEXT = "已取消导入";
const IMPORT_INVALID_JSON_ERROR_TEXT = "导入失败：文件不是有效的 JSON";
const IMPORT_SUCCESS_TEXT = "已导入 1 条学习数据";
const IMPORT_EMPTY_ERROR_TEXT = "导入失败：未找到可恢复的学习数据";
const IMPORT_SNAPSHOT_FAILURE_ERROR_TEXT = "导入失败：当前浏览器存储存在不可读数据，无法安全覆盖，请先清理后重试";
const IMPORT_WRITE_FAILURE_ERROR_TEXT = "导入失败：浏览器存储写入失败，已保留原有数据";
const IMPORT_ROLLBACK_FAILURE_ERROR_TEXT = "导入失败：浏览器存储异常，恢复过程中可能仅部分保留原有数据";
const CLEAR_SUCCESS_TEXT = "学习数据已清除";
const CLEAR_EMPTY_INFO_TEXT = "当前没有可清除的学习数据";
const EXPORT_EMPTY_INFO_TEXT = "暂无可导出的学习数据";
const MISSION_STEP_0_SUCCESS_TEXT = "步骤 1 已标记完成";
const MISSION_COMPLETION_TEXT_PREFIX = "任务完成：";
const MISSION_PROGRESS_SAVE_ERROR_TEXT = "保存学习进度失败，当前更改未写入浏览器存储。";
const MISSION_RESET_ROLLBACK_WARNING_TEXT = "任务进度未重置：未能清除该任务的进行中标记，已保留原进度。";
const MISSION_RESET_ROLLBACK_FAILURE_TEXT = "任务进度重置失败：未能清除进行中标记，且无法恢复原进度。";
const MISSION_MARKER_WARNING_TEXT = "当前浏览器存储存在异常任务标记，任务恢复可能不完整。";
const ETHICS_MISSION_ID = "mission-ethics";
const ETHICS_MISSION_CARD_SELECTOR = `.mission-card[data-mission="${ETHICS_MISSION_ID}"]`;
const ETHICS_TASK_NAME = "dashboard_mission_mission_ethics";
const ETHICS_TASK_KEY = `pm_metrics_task_start_P00-dashboard::${ETHICS_TASK_NAME}`;
const OVERVIEW_TASK_NAME = "learning_hub_overview_task";
const OVERVIEW_TASK_KEY = `pm_metrics_task_start_P00-dashboard::${OVERVIEW_TASK_NAME}`;
const UNREADABLE_PROGRESS_WARNING_TEXT = "当前浏览器存储存在异常学习数据，导出与覆盖恢复已停用。";
const STALE_TOAST_TEXT = "旧提示";
const ETHICS_STEP_0_PROGRESS = {
  [ETHICS_MISSION_ID]: { _started: true, step0: true }
};
const THEME_WRITE_PROBE_KEY = "__themeWriteProbe";
const THEME_UNREADABLE_PROBE_KEY = "__themeUnreadableProbe";
...
function countExactMatches(values, expected) {
  return values.filter((value) => value === expected).length;
}

async function readLatestDashboardSessionId(page) {
  ...
  }, DASHBOARD_EVENTS_KEY);
}

async function runThemePreferenceWarningProbe(page, options = {}) {
  return page.evaluate(async ({
    eventsKey,
    exportedDataToastText,
    themeStorageKey,
    themeWarningText,
    visibleAlertTextSelector,
    visibleToastSelector,
    seedPriorToast,
    disableReplaceToasts,
    unreadableExistingKey
  }) => {
    const readStatusTexts = (events) => events
      .filter((event) => event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
      .map((event) => event.status_text);
    const readVisibleAlertText = () => document.querySelector(visibleAlertTextSelector)?.textContent || "";

    let originalGetItem = null;
    let originalReplaceToasts = null;
    let originalSetItem = null;

    if (unreadableExistingKey) {
    ...
    if (seedPriorToast) {
      window.showToast(exportedDataToastText, "success", 3000);
      await new Promise((resolve) => setTimeout(resolve, 20));
    }

    if (disableReplaceToasts) {
      originalReplaceToasts = window.replaceToasts;
    ...
    try {
      ...
      return {
        ...
        statusTexts: readStatusTexts(events),
        storedTheme: unreadableExistingKey ? null : localStorage.getItem(themeStorageKey),
        storedThemeRaw: unreadableExistingKey && originalGetItem ? originalGetItem.call(localStorage, themeStorageKey) : "",
        toastText: readVisibleAlertText(),
        visibleToastCount: document.querySelectorAll(visibleToastSelector).length,
        visibleToastText: readVisibleAlertText(),
        warningTracked: events.some((event) => event.event_name === "status_error_signal" && event.status_text === themeWarningText)
      };
    } finally {
      if (originalReplaceToasts !== null) {
    ...
    eventsKey: DASHBOARD_EVENTS_KEY,
    exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
    themeStorageKey: THEME_STORAGE_KEY,
    themeWarningText: THEME_PREFERENCE_WARNING_TEXT,
    visibleAlertTextSelector: VISIBLE_ALERT_TEXT_SELECTOR,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR,
    seedPriorToast: options.seedPriorToast === true,
    disableReplaceToasts: options.disableReplaceToasts === true,
    unreadableExistingKey: options.unreadableExistingKey === true
  });
}

async function runExportSuccessReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    missionId,
    ...
    window.showToast(staleToastText, "success", 3000);
    await new Promise((resolve) => setTimeout(resolve, 20));
    ...
    disableReplaceToasts: options.disableReplaceToasts === true,
    eventsKey: DASHBOARD_EVENTS_KEY,
    missionId: ETHICS_MISSION_ID,
    progressKey: PROGRESS_KEY,
    staleToastText: STALE_TOAST_TEXT,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runMissionResetReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    exportedDataToastText,
...
async function runPopupBlockedReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    missionId,
...
async function runUnreadableProgressWarningProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    missionId,
    ...
    try {
      ...
      const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
      return {
        toastText: document.querySelector(visibleAlertTextSelector)?.textContent || "",
        priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === staleToastText).length,
        warningSignalCount: events.filter((event) =>
          (event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
          && event.status_text === warningText
        ).length,
        ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
        visibleToastCount: document.querySelectorAll(visibleToastSelector).length
      };
    ...
    visibleAlertTextSelector: VISIBLE_ALERT_TEXT_SELECTOR,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR,
    warningText: UNREADABLE_PROGRESS_WARNING_TEXT
  });
}

async function runImportCancelReplacementProbe(page, options = {}) {
  await page.evaluate(({ disableReplaceToasts, exportedDataToastText }) => {
    localStorage.clear();
  ...
  try {
    ...
    await page.setInputFiles("#importInput", {
      ...
      buffer: Buffer.from(JSON.stringify({
        [PROGRESS_KEY]: JSON.stringify({
          [ETHICS_MISSION_ID]: { _started: true }
        })
      }))
    });
    await page.waitForFunction((message) => document.querySelector("#toastContainer")?.textContent?.includes(message), IMPORT_CANCEL_INFO_TEXT);

    return page.evaluate(({ cancelInfoText, eventsKey, exportedDataToastText, progressKey, visibleToastSelector }) => {
      window.pmMetrics.track("cta_click", { control_id: "probe" });
      const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
      return {
        priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === exportedDataToastText).length,
        cancelSignalCount: events.filter((event) =>
          (event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
          && event.status_text === cancelInfoText
        ).length,
        ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
      ...
      cancelInfoText: IMPORT_CANCEL_INFO_TEXT,
      eventsKey: DASHBOARD_EVENTS_KEY,
      exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
      progressKey: PROGRESS_KEY,
      visibleToastSelector: VISIBLE_TOAST_SELECTOR
    ...
    if (options.disableReplaceToasts === true) {
      await page.evaluate(() => {
...
async function runImportInvalidJsonReplacementProbe(page, options = {}) {
  await page.evaluate(({ disableReplaceToasts, exportedDataToastText }) => {
    localStorage.clear();
    window.showToast(exportedDataToastText, "success", 3000);
    if (disableReplaceToasts) {
  ...
  try {
    ...
    await page.waitForFunction((message) => document.querySelector("#toastContainer")?.textContent?.includes(message), IMPORT_INVALID_JSON_ERROR_TEXT);

    return page.evaluate(({ errorText, eventsKey, exportedDataToastText, visibleToastSelector }) => {
      window.pmMetrics.track("cta_click", { control_id: "probe" });
      const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
      return {
        priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === exportedDataToastText).length,
        importErrorCount: events.filter((event) => event.event_name === "status_error_signal" && event.status_text === errorText).length,
        ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
        visibleToastCount: document.querySelectorAll(visibleToastSelector).length
      };
    }, {
      errorText: IMPORT_INVALID_JSON_ERROR_TEXT,
      eventsKey: DASHBOARD_EVENTS_KEY,
      exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
      visibleToastSelector: VISIBLE_TOAST_SELECTOR
    });
...
async function runImportErrorReplacementProbe(page, {
  errorText,
  fileBuffer,
  fileName,
  includeEventsRaw = false,
  includeProgressRaw = false,
  setupPage
}) {
  ...
  await page.waitForFunction((message) => document.querySelector("#toastContainer")?.textContent?.includes(message), errorText);

  return page.evaluate(({ errorText: currentErrorText, eventsKey, exportedDataToastText, progressKey, readEventsRaw, readProgressRaw, visibleToastSelector }) => {
    window.pmMetrics.track("cta_click", { control_id: "probe" });
    const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
    return {
      priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === exportedDataToastText).length,
      importErrorCount: events.filter((event) => event.event_name === "status_error_signal" && event.status_text === currentErrorText).length,
      ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
      progressRaw: readProgressRaw ? localStorage.getItem(progressKey) : "",
      eventsRaw: readEventsRaw ? localStorage.getItem(eventsKey) : "",
      visibleToastCount: document.querySelectorAll(visibleToastSelector).length
    };
  }, {
    errorText,
    ...
    exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
    progressKey: PROGRESS_KEY,
    readEventsRaw: includeEventsRaw === true,
    readProgressRaw: includeProgressRaw === true,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runImportInputSuccessReplacementProbe(page, options = {}) {
  await page.evaluate(({ disableReplaceToasts, exportedDataToastText }) => {
    localStorage.clear();
    window.confirm = () => true;
    window.showToast(exportedDataToastText, "success", 3000);
  ...
  try {
    ...
    await page.waitForFunction((message) => document.querySelector("#toastContainer")?.textContent?.includes(message), IMPORT_SUCCESS_TEXT);

    return page.evaluate(({ eventsKey, successText, visibleToastSelector }) => {
      window.pmMetrics.track("cta_click", { control_id: "probe" });
      const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
      return {
        statusTexts: events.filter((event) => event.event_name === "status_success_signal").map((event) => event.status_text),
      ...
      eventsKey: DASHBOARD_EVENTS_KEY,
      successText: IMPORT_SUCCESS_TEXT,
      visibleToastSelector: VISIBLE_TOAST_SELECTOR
    });
  } finally {
    if (options.disableReplaceToasts === true) {
...
async function runExportEmptyReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    infoText,
    ...
    localStorage.clear();
    window.showToast(staleToastText, "success", 3000, { track: trackPriorToast });
    await new Promise((resolve) => setTimeout(resolve, 20));
    ...
    try {
      ...
      return {
        priorToastCount: events.filter((event) =>
          (event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
          && event.status_text === staleToastText
        ).length,
        exportInfoSignalCount: events.filter((event) =>
          (event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
          && event.status_text === infoText
        ).length,
        ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
        visibleToastCount: document.querySelectorAll(visibleToastSelector).length
      };
    ...
    disableReplaceToasts: options.disableReplaceToasts === true,
    eventsKey: DASHBOARD_EVENTS_KEY,
    infoText: EXPORT_EMPTY_INFO_TEXT,
    staleToastText: STALE_TOAST_TEXT,
    trackPriorToast: options.trackPriorToast !== false,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runClearEmptyReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    infoText,
    ...
    localStorage.clear();
    window.showToast(staleToastText, "success", 3000, { track: false });
    await new Promise((resolve) => setTimeout(resolve, 20));
    window.confirm = () => true;
    ...
    disableReplaceToasts: options.disableReplaceToasts === true,
    eventsKey: DASHBOARD_EVENTS_KEY,
    infoText: CLEAR_EMPTY_INFO_TEXT,
    staleToastText: STALE_TOAST_TEXT,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runClearSuccessReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    missionId,
    ...
    missionId: ETHICS_MISSION_ID,
    progressKey: PROGRESS_KEY,
    successText: CLEAR_SUCCESS_TEXT,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runUnreadableMissionMarkerWarningProbe(page, options = {}) {
  return page.evaluate(async ({
    disableReplaceToasts,
    eventsKey,
    missionId,
    ...
    seedPriorToast: options.seedPriorToast === true,
    staleToastText: STALE_TOAST_TEXT,
    taskKey: ETHICS_TASK_KEY,
    visibleAlertTextSelector: VISIBLE_ALERT_TEXT_SELECTOR,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR,
    warningText: MISSION_MARKER_WARNING_TEXT
  });
}

async function runMissionStepSuccessReplacementProbe(page, options = {}) {
  return page.evaluate(async ({
    completeMission,
    eventsKey,
    missionId,
    ...
    completeMission: options.completeMission === true,
    eventsKey: DASHBOARD_EVENTS_KEY,
    missionId: ETHICS_MISSION_ID,
    staleToastText: STALE_TOAST_TEXT,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
...
async function runMissionStepPersistenceFailureProbe(page, options = {}) {
  return page.evaluate(async ({
    blockOnlyStepWrite,
    eventsKey,
    exportedDataToastText,
    ...
    Storage.prototype.setItem = function (key, value) {
      if (key === progressKey && (!blockOnlyStepWrite || String(value).includes("\"step0\":true"))) {
        return;
      }
      return originalSetItem.call(this, key, value);
    };
    ...
    try {
      ...
      return {
        priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === exportedDataToastText).length,
        saveErrorCount: events.filter((event) => event.event_name === "status_error_signal" && event.status_text === saveErrorText).length,
    ...
    blockOnlyStepWrite: options.blockOnlyStepWrite === true,
    eventsKey: DASHBOARD_EVENTS_KEY,
    exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
    missionId: ETHICS_MISSION_ID,
    progressKey: PROGRESS_KEY,
    saveErrorText: MISSION_PROGRESS_SAVE_ERROR_TEXT,
    taskName: ETHICS_TASK_NAME,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR
  });
}

async function runMissionResetRollbackFailureProbe(page, options = {}) {
  return page.evaluate(async ({
    eventsKey,
    exportedDataToastText,
    failRollbackRestore,
    ...
    Storage.prototype.removeItem = function (key) {
      if (key === taskKey) {
        throw new Error("forced marker remove failure");
      }
      return originalRemoveItem.call(this, key);
    };
    if (failRollbackRestore) {
      Storage.prototype.setItem = function (key, value) {
        if (key === progressKey && String(value).includes(restoreMissionId)) {
          return;
        }
        return originalSetItem.call(this, key, value);
      };
    ...
    try {
      ...
      return {
        priorToastCount: events.filter((event) => event.event_name === "status_success_signal" && event.status_text === exportedDataToastText).length,
        resetErrorCount: events.filter((event) =>
          event.event_name === "status_error_signal"
          && event.status_text === (failRollbackRestore ? failureText : warningText)
        ).length,
        ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length,
    ...
    eventsKey: DASHBOARD_EVENTS_KEY,
    exportedDataToastText: EXPORTED_DATA_TOAST_TEXT,
    failRollbackRestore: options.failRollbackRestore === true,
    failureText: MISSION_RESET_ROLLBACK_FAILURE_TEXT,
    missionId: ETHICS_MISSION_ID,
    progressKey: PROGRESS_KEY,
    restoreMissionId: ETHICS_MISSION_ID,
    taskKey: ETHICS_TASK_KEY,
    visibleToastSelector: VISIBLE_TOAST_SELECTOR,
    warningText: MISSION_RESET_ROLLBACK_WARNING_TEXT
  });
}

async function runMissionResetRollbackStateProbe(page, options = {}) {
  return page.evaluate(({ missionId, silentNoOp, taskKey, progressKey }) => {
    localStorage.clear();
    window.ensureMissionStarted(missionId);
    window.openMissionModal(missionId);
    ...
    progressKey: PROGRESS_KEY,
    taskKey: ETHICS_TASK_KEY,
    silentNoOp: options.silentNoOp === true
  });
}

async function runClearRollbackStateProbe(page, options = {}) {
  return page.evaluate(({ eventsKey, missionId, progressKey, silentNoOp }) => {
    localStorage.clear();
    localStorage.setItem(progressKey, JSON.stringify({
      [missionId]: { _started: true }
    }));
    localStorage.setItem(eventsKey, JSON.stringify([{
      ...
      session_id: silentNoOp ? "clear-silent-rollback" : "clear-rollback",
      app_version: "pm-v1",
      page_path: "/"
    }]));
    ...
    Storage.prototype.removeItem = function (key) {
      if (key === eventsKey) {
        if (silentNoOp) return;
        throw new Error("forced clear remove failure");
      }
      return originalRemoveItem.call(this, key);
    };
...
async function runTaskTerminalMarkerProbe(page, options = {}) {
  return page.evaluate(({ action, eventsKey, taskKey, taskName, unreadableAfterRemove }) => {
    localStorage.clear();
    localStorage.setItem(taskKey, String(Date.now() - 2000));
    ...
    try {
      ...
      return {
        ...
        markerAfter: originalGetItem.call(localStorage, taskKey),
    ...
    action: options.action === "error" ? "error" : "complete",
    eventsKey: DASHBOARD_EVENTS_KEY,
    taskKey: OVERVIEW_TASK_KEY,
    taskName: OVERVIEW_TASK_NAME,
    unreadableAfterRemove: options.unreadableAfterRemove === true
  });
}

async function runTaskStartBackfillRecoveryCase(browser, origin, action = "complete") {
  const page = await browser.newPage();
  await page.addInitScript((eventsKey) => {
    const originalSetItem = Storage.prototype.setItem;
    let failed = false;
    Storage.prototype.setItem = function (key, value) {
      if (key === eventsKey && !failed && String(value).includes("\"task_start\"")) {
        failed = true;
  ...
  try {
    return await page.evaluate(({ eventsKey, nextAction, taskName }) => {
      localStorage.clear();
      window.pmMetrics.markTaskStart(taskName);
      if (nextAction === "error") {
        window.pmMetrics.markTaskError(taskName, "boom");
      } else {
        window.pmMetrics.markTaskComplete(taskName, { done: true });
    ...
    }, { eventsKey: DASHBOARD_EVENTS_KEY, nextAction: action, taskName: OVERVIEW_TASK_NAME });
  } finally {
    await page.close();
  }
}

async function runTaskBackfillRetentionProbe(page, action = "complete") {
  return page.evaluate(({ eventsKey, nextAction, taskKey, taskName }) => {
...
async function runTaskBackfillAtomicityCase(browser, origin, action = "complete") {
  const page = await browser.newPage();
  await page.addInitScript(({ errorMessage, eventsKey }) => {
    const originalSetItem = Storage.prototype.setItem;
    let failed = false;
    Storage.prototype.setItem = function (key, value) {
      if (key === eventsKey && !failed && String(value).includes("\"task_start\"")) {
    ...
    errorMessage: action === "error"
      ? "forced combined task_start/task_error write failure"
      : "forced combined task_start/task_complete write failure"
  });
  const response = await page.goto(`${origin}/`, { waitUntil: "networkidle" });
  assert(response && response.status() === 200, `Unexpected HTTP status: ${response ? response.status() : "none"}`);
  try {
    return await page.evaluate(({ eventsKey, nextAction, taskKey, taskName }) => {
      localStorage.clear();
      localStorage.setItem(taskKey, String(Date.now() - 2000));
      if (nextAction === "error") {
        window.pmMetrics.markTaskError(taskName, "boom");
...
async function createPageWithEventWriteFailure(context, origin, { errorMessage, trackFailures = false, trackedEventSnippet, failureCounterKey = "" }) {
  await context.addInitScript(({ counterKey, nextErrorMessage, nextEventsKey, nextTrackFailures, nextTrackedEventSnippet }) => {
    const originalSetItem = Storage.prototype.setItem;
    let failed = false;
    if (counterKey) {
      window[counterKey] = 0;
    ...
    Storage.prototype.setItem = function (key, value) {
      if (key === nextEventsKey && !failed && String(value).includes(nextTrackedEventSnippet)) {
        failed = true;
        if (nextTrackFailures && counterKey) {
          window[counterKey] += 1;
        }
    ...
    counterKey: failureCounterKey,
    nextErrorMessage: errorMessage,
    nextEventsKey: DASHBOARD_EVENTS_KEY,
    nextTrackFailures: trackFailures === true,
    nextTrackedEventSnippet: trackedEventSnippet
  });
  return createReadyPage(context, origin);
}

async function createPageWithPageViewWriteFailure(context, origin) {
  return createPageWithEventWriteFailure(context, origin, {
    errorMessage: "forced page_view write failure",
    trackedEventSnippet: "\"page_view\""
  });
}

async function createPageWithStatusSuccessWriteFailure(context, origin, options = {}) {
  return createPageWithEventWriteFailure(context, origin, {
    errorMessage: options.errorMessage || "forced status_success_signal write failure",
    trackFailures: options.trackFailures === true,
    trackedEventSnippet: "\"status_success_signal\"",
    failureCounterKey: "__pmStatusWriteFailures"
  });
}

async function createPageWithFirstInteractionWriteFailure(context, origin) {
  return createPageWithEventWriteFailure(context, origin, {
    errorMessage: "forced first_interaction write failure",
    trackedEventSnippet: "\"first_interaction\""
  });
}

async function runStatusSignalResyncProbe(page, mode, source) {
  return page.evaluate(async ({ currentMode, currentSource, eventsKey, missionId, progressKey }) => {
    if (currentSource === "status") {
...
async function runQueuedStatusSignalProbe(page, options = {}) {
  return page.evaluate(async ({ eventsKey, resetAfterIntermediate }) => {
    const status = document.createElement("div");
    status.id = "status";
    document.body.appendChild(status);
    status.textContent = "导出成功";
    await new Promise((resolve) => setTimeout(resolve, 0));
    status.textContent = resetAfterIntermediate ? "处理中" : "操作失败";
    await new Promise((resolve) => setTimeout(resolve, 0));
    if (resetAfterIntermediate) {
      status.textContent = "导出成功";
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    window.pmMetrics.track("cta_click", { control_id: "probe" });
    ...
    return {
      statusNames: events
        .filter((event) => resetAfterIntermediate
          ? event.event_name === "status_success_signal"
          : event.event_name === "status_success_signal" || event.event_name === "status_error_signal")
        .map((event) => resetAfterIntermediate ? event.status_text : `${event.event_name}:${event.status_text}`),
      ctaClickCount: events.filter((event) => event.event_name === "cta_click" && event.control_id === "probe").length
    };
  }, {
    eventsKey: DASHBOARD_EVENTS_KEY,
    resetAfterIntermediate: options.resetAfterIntermediate === true
  });
}

async function runToastObserverProbe(page, message, type, expectedEventName) {
  return page.evaluate(async ({ eventName, eventsKey, nextMessage, nextType }) => {
    window.showToast(nextMessage, nextType, 3000);
    await new Promise((resolve) => setTimeout(resolve, 50));
    const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
    const statusSignals = events.filter((event) => event.event_name === eventName);
    return {
...
async function setMissionProgress(page, progress) {
  await page.evaluate(({ nextProgress, progressKey }) => {
    localStorage.setItem(progressKey, JSON.stringify(nextProgress));
  }, { nextProgress: progress, progressKey: PROGRESS_KEY });
}
...
async function openEthicsModal(page, startLabelIncludes = "") {
  await page.click(`.mission-card[data-mission="${ETHICS_MISSION_ID}"]`);
...
async function installUnreadableThemeStorage(context, seedValue = null) {
  await context.addInitScript(({ initialValue, storageKey }) => {
  ...
  }, { initialValue: seedValue, storageKey: THEME_STORAGE_KEY });
}
...
async function readThemeState(page) {
  return page.evaluate((storageKey) => {
    ...
    return {
      ...
      buttonText: document.getElementById("darkToggleBtn")?.textContent || "",
...
async function assertStartupThemeCase(context, origin, pageFactory, factoryOptions, expected, prefix) {
  const page = await pageFactory(context, origin, factoryOptions);
  try {
    await assertCurrentThemeState(page, expected, prefix);
  } finally {
    await page.close();
  }
}
...
async function setMockSystemTheme(page, nextTheme) {
  await page.evaluate((theme) => {
    window.__setMockSystemTheme?.(theme);
  }, nextTheme);
}

async function switchMockSystemTheme(page, nextTheme, expectedTheme = nextTheme) {
  await setMockSystemTheme(page, nextTheme);
  await waitForTheme(page, expectedTheme);
}

async function setStoredTheme(page, value) {
  await page.evaluate(({ nextValue, storageKey }) => {
    if (nextValue === null) {
      localStorage.removeItem(storageKey);
      return;
    }
    localStorage.setItem(storageKey, nextValue);
  }, { nextValue: value, storageKey: THEME_STORAGE_KEY });
}
...
async function createNoMatchMediaThemePage(context, origin, { storedTheme, unreadableThemeSeed } = {}) {
  await installNoMatchMedia(context);
  if (unreadableThemeSeed !== undefined) {
    await installUnreadableThemeStorage(context, unreadableThemeSeed);
  }
  ...
  if (storedTheme !== undefined) {
    await reloadThemePage(page, { storedTheme });
  }
  return page;
}
...
async function createSystemThemePage(context, origin, initialTheme, options = {}) {
  await installSystemThemeMock(context, initialTheme, { legacyListener: options.legacyListener === true });
  if (options.unreadableThemeSeed !== undefined) {
    await installUnreadableThemeStorage(context, options.unreadableThemeSeed);
  }
  return createReadyPage(context, origin);
}

async function createStartupThemePage(context, origin, options = {}) {
  const systemTheme = options.systemTheme ?? null;
  const noMatchMedia = options.noMatchMedia === true;
  const unreadableThemeSeed = options.unreadableThemeSeed;
  const storedTheme = options.storedTheme;
  const colorScheme = options.colorScheme ?? null;

  if (systemTheme) {
    await installSystemThemeMock(context, systemTheme);
  } else if (noMatchMedia) {
    await installNoMatchMedia(context);
  }
  if (unreadableThemeSeed !== undefined) {
    await installUnreadableThemeStorage(context, unreadableThemeSeed);
  ...
  const page = await createReadyPage(context, origin);
  if (storedTheme !== undefined || colorScheme !== null || unreadableThemeSeed !== undefined) {
    await reloadThemePage(page, { colorScheme, storedTheme });
  }
  return page;
}

async function withPage(pagePromise, runCase) {
  const page = await pagePromise;
  try {
    return await runCase(page);
  } finally {
    await page.close();
  }
}

async function withPagePair(pairPromise, runCase) {
  const pages = await pairPromise;
  try {
    return await runCase(pages);
  } finally {
    await pages.pageA.close();
    await pages.pageB.close();
  }
}

async function installThemeMutationProbe(page, { probeKey, makeStoredThemeUnreadable = false }) {
  await page.evaluate(({ currentProbeKey, unreadableStoredTheme, themeKey }) => {
    const originalGetItem = Storage.prototype.getItem;
    const originalSetAttribute = Element.prototype.setAttribute;
    let themeWriteCount = 0;
    window[currentProbeKey] = {
      read() {
      ...
      restore() {
        ...
        delete window[currentProbeKey];
      }
    };
    if (unreadableStoredTheme) {
      Storage.prototype.getItem = function (key) {
        if (key === themeKey) {
          throw new Error("forced unreadable theme key");
        }
  ...
  }, { currentProbeKey: probeKey, unreadableStoredTheme: makeStoredThemeUnreadable, themeKey: THEME_STORAGE_KEY });
}

async function readAndRestoreThemeMutationProbe(page, probeKey) {
  const probe = await page.evaluate((currentProbeKey) => window[currentProbeKey]?.read?.() || { themeWriteCount: -1 }, probeKey);
  const state = await readThemeState(page);
  await page.evaluate((currentProbeKey) => window[currentProbeKey]?.restore?.(), probeKey);
  return { probe, state };
}

async function assertCrossTabUnreadableThemeFallback(pageA, pageB, expectedThemeState, prefix) {
  await installThemeMutationProbe(pageB, { probeKey: THEME_UNREADABLE_PROBE_KEY, makeStoredThemeUnreadable: true });
  await setStoredTheme(pageA, "dark");
  await pageB.waitForTimeout(150);

  const { probe, state } = await readAndRestoreThemeMutationProbe(pageB, THEME_UNREADABLE_PROBE_KEY);
  assert(probe.themeWriteCount > 0, `${prefix} should still trigger theme reapplication writes in the receiving tab: ${JSON.stringify({ probe, state })}`);
  assertThemeUiState(state, expectedThemeState, prefix);
  return { probe, state };
}

function readMockSystemThemeRegistration(page) {
  return page.evaluate(() => window.__readMockSystemThemeRegistration?.() || null);
}
...
async function run() {
  ...
  try {
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey }) => {
          localStorage.clear();
        ...
        return passedCase("dualFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("replacementCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate((eventsKey) => {
          localStorage.clear();
        ...
        return passedCase("metricsSilentWriteFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("metricsCorruptExistingKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("metricsInvalidArrayExistingKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("metricsUnreadableExistingKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, taskKey, taskName }) => {
          localStorage.clear();
          ...
          try {
            const started = window.pmMetrics.markTaskStart(taskName);
            return {
        ...
        }, { eventsKey: DASHBOARD_EVENTS_KEY, taskKey: OVERVIEW_TASK_KEY, taskName: OVERVIEW_TASK_NAME });
        ...
        return passedCase("taskStartMarkerSilentWriteFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartMarkerUnreadableExistingKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey, taskName }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, JSON.stringify({ [missionId]: { _started: true } }));
          ...
          try {
            ...
            return {
              callCount,
              taskStartEvents: events.filter(event => event.event_name === "task_start" && event.task_name === taskName).length
            };
          } finally {
            Storage.prototype.setItem = originalSetItem;
            window.pmMetrics.markTaskStart = originalMarkTaskStart;
        ...
        return passedCase("unrelatedMetricCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("initialStartCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, otherMissionId, progressKey, taskKey, taskName }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, JSON.stringify({ [missionId]: { _started: true } }));
          ...
          eventsKey: DASHBOARD_EVENTS_KEY,
          missionId: ETHICS_MISSION_ID,
          otherMissionId: "mission-ai-content",
          progressKey: PROGRESS_KEY,
          taskKey: ETHICS_TASK_KEY,
          taskName: ETHICS_TASK_NAME
        });
        ...
        return passedCase("unrelatedProgressCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runThemePreferenceWarningProbe(page);
        ...
        assert(result.toastText === THEME_PREFERENCE_WARNING_TEXT, `Dark toggle did not surface the preference write warning toast: ${JSON.stringify(result)}`);
        assert(result.warningTracked === true, `Dark toggle storage warning was not tracked as a status signal: ${JSON.stringify(result)}`);
        return passedCase("themePreferenceSilentWriteFailureCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runThemePreferenceWarningProbe(page, { seedPriorToast: true });
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Theme warning should not duplicate a prior tracked toast: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, THEME_PREFERENCE_WARNING_TEXT) === 1, `Theme warning should record exactly one warning signal: ${JSON.stringify(result)}`);
        ...
        assert(result.visibleToastText === THEME_PREFERENCE_WARNING_TEXT, `Theme warning did not remain as the only visible toast: ${JSON.stringify(result)}`);
        return passedCase("themePreferenceDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runThemePreferenceWarningProbe(page, { seedPriorToast: true, disableReplaceToasts: true });
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Theme warning fallback should not duplicate a prior tracked toast: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, THEME_PREFERENCE_WARNING_TEXT) === 1, `Theme warning fallback should record exactly one warning signal: ${JSON.stringify(result)}`);
        ...
        assert(result.visibleToastText === THEME_PREFERENCE_WARNING_TEXT, `Theme warning fallback did not leave the warning as the only visible toast: ${JSON.stringify(result)}`);
        return passedCase("themePreferenceFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runThemePreferenceWarningProbe(page, { unreadableExistingKey: true });
        ...
        assert(result.toastText === THEME_PREFERENCE_WARNING_TEXT, `Dark toggle did not warn on unreadable existing theme storage: ${JSON.stringify(result)}`);
        ...
        return passedCase("themePreferenceUnreadableExistingKeyCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("themeColorMetaSyncCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabFocusRestoreCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabClearFocusRestoreCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabManagedKeyFilterCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("refreshQueueCoalescingCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("focusRefreshCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("visibilityRefreshGuardCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("tabKeyboardNavigationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        await page.focus(ETHICS_MISSION_CARD_SELECTOR);
        ...
        assert(result.restoredFocusMission === ETHICS_MISSION_ID && result.restoredFocusRole === "button", `Closing the mission modal should restore focus to the launching mission card: ${JSON.stringify(result)}`);
        return passedCase("missionModalFocusManagementCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("missionCardSpaceActivationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        await page.click(ETHICS_MISSION_CARD_SELECTOR);
        await page.waitForFunction(() => document.getElementById("missionModal")?.classList.contains("open"));
        ...
        assert(closeButtonResult.restoredFocusMission === ETHICS_MISSION_ID && closeButtonResult.restoredFocusRole === "button", `Close button should restore focus to the launching mission card: ${JSON.stringify(closeButtonResult)}`);
        ...
        assert(backdropResult.restoredFocusMission === ETHICS_MISSION_ID && backdropResult.restoredFocusRole === "button", `Backdrop click should restore focus to the launching mission card: ${JSON.stringify(backdropResult)}`);
        return passedCase("missionModalCloseControlsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ missionId }) => {
          const mission = MISSIONS.find((item) => item.id === missionId);
          const buildSnapshot = () => ({
        ...
        return passedCase("missionModalCopyStatesCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("missionModalStepRenderingCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(async ({ missionId, progressKey }) => {
          const originalOpen = window.open;
          let openedUrl = "";
          window.open = (url) => {
        ...
        return passedCase("missionModalStartResumesNextStepCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("missionModalStartReopensFirstStepCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ missionId, missionSelector }) => {
          const mission = MISSIONS.find((item) => item.id === missionId);
          const totalSteps = mission?.steps?.length || 0;

          function snapshot() {
            const card = document.querySelector(missionSelector);
        ...
        }, { missionId: ETHICS_MISSION_ID, missionSelector: ETHICS_MISSION_CARD_SELECTOR });
        ...
        return passedCase("missionCardProgressStateCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        assert(result.activeMission === ETHICS_MISSION_ID && result.activeRole === "button", `Dashboard refresh should restore focus to the previously focused mission card: ${JSON.stringify(result)}`);
        return passedCase("pageFocusRestoreCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("tabFocusRestoreCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("toolLinkFocusRestoreCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("idFocusRestoreCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("anchorFocusRestoreCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("modalFocusPreservationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        await openEthicsModal(page);
        await page.evaluate(({ missionId, progressKey }) => {
          const stepLink = document.querySelector('.tool-link[data-step="0"]');
          if (stepLink instanceof HTMLElement) {
            stepLink.focus();
          }
        ...
        return passedCase("modalFocusFallbackCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabModalStepLinkPreservationCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabModalStepLinkFallbackCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabModalFocusPreservationCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabModalClearResetCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabThemeSyncCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabThemeResetCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const { pageA, pageB } = await createPagePair(context, origin);
      try {
        await seedCrossTabDarkTheme(pageA, pageB, "dark");
        await setCrossTabStoredTheme(pageA, pageB, "light", "light");
        await setCrossTabStoredTheme(pageA, pageB, null, "dark");

        await assertCurrentThemeState(pageB, darkThemeExpectation(null), "Cross-tab theme reset with dark system theme");
        return passedCase("crossTabThemeResetSystemDarkCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabThemeClearResetCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        await assertCurrentThemeState(pageB, darkThemeExpectation(null), "Cross-tab theme clear with dark system theme");
        return passedCase("crossTabThemeClearSystemDarkCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("crossTabInvalidThemeFallbackCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const { pageA, pageB } = await createPagePair(context, origin);
      try {
        await emulateAndReload([pageA, pageB], "dark");
        await setCrossTabStoredTheme(pageA, pageB, "light", "light");
        await setCrossTabStoredTheme(pageA, pageB, "sepia", "dark");

        await assertCurrentThemeState(pageB, darkThemeExpectation("sepia"), "Cross-tab invalid theme storage with dark system theme");
        return passedCase("crossTabInvalidThemeSystemDarkCase");
      } finally {
        await pageA.close();
        await pageB.close();
      }
    ...
    for (const startupThemeCase of [
      {
        name: "startupInvalidThemeFallbackCase",
        options: { colorScheme: "light", storedTheme: "sepia" },
        expected: lightThemeExpectation("sepia"),
        prefix: "Startup invalid theme storage"
      },
      {
        name: "startupInvalidThemeSystemDarkCase",
        options: { systemTheme: "dark", storedTheme: "sepia" },
        expected: darkThemeExpectation("sepia"),
        prefix: "Startup invalid theme storage with dark system theme"
      },
      {
        name: "startupStoredThemeOverrideCase",
        options: { colorScheme: "light", storedTheme: "dark" },
        expected: darkThemeExpectation("dark"),
        prefix: "Startup stored theme"
      },
      {
      ...
      {
        name: "noMatchMediaInvalidThemeFallbackCase",
        options: { noMatchMedia: true, storedTheme: "sepia" },
        expected: lightThemeExpectation("sepia"),
        prefix: "No-matchMedia invalid theme storage"
      },
      {
      ...
      {
        name: "noMatchMediaPinnedThemeCase",
        options: { noMatchMedia: true, storedTheme: "dark" },
        expected: darkThemeExpectation("dark"),
        prefix: "No-matchMedia pinned theme"
      }
    ]) {
      results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
        await assertStartupThemeCase(
          context,
          origin,
          createStartupThemePage,
          startupThemeCase.options,
          startupThemeCase.expected,
          startupThemeCase.prefix
        );
        return passedCase(startupThemeCase.name);
      }));
    }

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createNoMatchMediaPage(context, origin), async (page) => {
        await page.waitForSelector("#darkToggleBtn");
        await page.click("#darkToggleBtn");
        await waitForTheme(page, "dark");
        await assertCurrentThemeState(page, darkThemeExpectation("dark"), "No-matchMedia toggle click");
        return passedCase("noMatchMediaTogglePersistCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createNoMatchMediaPagePair(context, origin), async ({ pageA, pageB }) => {
        ...
        return passedCase("noMatchMediaCrossTabThemeSyncCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createNoMatchMediaPagePair(context, origin), async ({ pageA, pageB }) => {
        ...
        return passedCase("noMatchMediaCrossTabThemeResetCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createNoMatchMediaPagePair(context, origin), async ({ pageA, pageB }) => {
        ...
        return passedCase("noMatchMediaCrossTabClearCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createNoMatchMediaPagePair(context, origin), async ({ pageA, pageB }) => {
        ...
        return passedCase("noMatchMediaCrossTabInvalidThemeCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createPagePair(context, origin), async ({ pageA, pageB }) => {
        await emulateAndReload([pageA, pageB], "dark");
        await setCrossTabStoredTheme(pageA, pageB, "light", "light");
        await assertCrossTabUnreadableThemeFallback(pageA, pageB, darkThemeExpectation("__unreadable__"), "Cross-tab unreadable theme storage with dark system theme");
        return passedCase("crossTabUnreadableThemeSystemDarkCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPagePair(createPagePair(context, origin), async ({ pageA, pageB }) => {
        ...
        await installThemeMutationProbe(pageB, { probeKey: THEME_WRITE_PROBE_KEY });
        await pageA.evaluate(() => {
          localStorage.setItem("unrelated_theme_probe_key", "1");
        });
        await pageB.waitForTimeout(150);

        const { probe, state } = await readAndRestoreThemeMutationProbe(pageB, THEME_WRITE_PROBE_KEY);
        ...
        return passedCase("crossTabThemeIgnoresUnrelatedStorageCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createSystemThemePage(context, origin, "light"), async (page) => {
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createSystemThemePage(context, origin, "light"), async (page) => {
        ...
        await assertCurrentThemeState(page, lightThemeExpectation("sepia"), "System theme with invalid stored theme");
        await switchMockSystemTheme(page, "dark");
        await assertCurrentThemeState(page, darkThemeExpectation("sepia"), "System theme changes with invalid stored theme");
        return passedCase("systemThemeInvalidStoredCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createSystemThemePage(context, origin, "light", { legacyListener: true }), async (page) => {
        const before = await readMockSystemThemeRegistration(page);
        await switchMockSystemTheme(page, "dark");
        const result = await readThemeState(page);
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createSystemThemePage(context, origin, "light"), async (page) => {
        ...
        await page.click("#darkToggleBtn");
        await waitForTheme(page, "dark");
        await setMockSystemTheme(page, "light");
        await sleep();
        const result = await readThemeState(page);
        assertThemeUiState(result, darkThemeExpectation("dark"), "Stored theme preferences");
        return passedCase("systemThemeIgnorePinnedCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("moduleRestoredBadgeCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("moduleUsedBadgePrecedenceCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ missionSelector }) => {
          const missionCard = document.querySelector(missionSelector);
        ...
        }, { missionSelector: ETHICS_MISSION_CARD_SELECTOR });
        ...
        return passedCase("interactiveSemanticsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statsSelfMetricsOnlyEmptyStateCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statsRestoredProgressEmptyStateCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statsRestoredProgressWithoutSelfMetricsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statsPositiveRenderingCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey }) => {
          const mission = MISSIONS.find((item) => item.id === missionId);
        ...
        return passedCase("heroStatsAggregationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("offlineShellCase");
      } finally {
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      assert(consoleErrors.length === 0, `Console errors: ${consoleErrors.join(" | ")}`);
      try {
        const result = await page.evaluate(async ({ eventsKey, visibleToastSelector }) => {
          await new Promise(resolve => setTimeout(resolve, 50));
          window.pmMetrics.track("cta_click", { control_id: "probe" });
          const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
          return {
        ...
        }, { eventsKey: DASHBOARD_EVENTS_KEY, visibleToastSelector: VISIBLE_TOAST_SELECTOR });
        ...
        return passedCase("serviceWorkerRegisterFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        assert(result.exportedKeys.includes(PROGRESS_KEY), `Export payload missing progress after unreadable task-start key: ${JSON.stringify(result.exportedKeys)}`);
        ...
        return passedCase("unreadableTaskKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("exportButtonBuildFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(async ({ eventsKey, progressKey }) => {
          localStorage.clear();
        ...
        }, { eventsKey: DASHBOARD_EVENTS_KEY, progressKey: PROGRESS_KEY });
        ...
        return passedCase("exportButtonUnreadableDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runExportEmptyReplacementProbe(page);
        ...
        return passedCase("exportButtonEmptyDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runExportEmptyReplacementProbe(page, { disableReplaceToasts: true, trackPriorToast: false });
        ...
        return passedCase("exportButtonEmptyFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runExportSuccessReplacementProbe(page);
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Export success should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Export success should record exactly one export success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("exportButtonDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runExportSuccessReplacementProbe(page, { disableReplaceToasts: true });
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Export success fallback should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Export success fallback should record exactly one export success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("exportButtonFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runPopupBlockedReplacementProbe(page);
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Popup warning should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, POPUP_BLOCKED_WARNING_TEXT) === 1, `Popup warning should record exactly one warning signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("popupBlockedDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runPopupBlockedReplacementProbe(page, { disableReplaceToasts: true });
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Popup warning fallback should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, POPUP_BLOCKED_WARNING_TEXT) === 1, `Popup warning fallback should record exactly one warning signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("popupBlockedFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runUnreadableProgressWarningProbe(page);
        assert(result.toastText === UNREADABLE_PROGRESS_WARNING_TEXT, `Unreadable backup-critical storage warning toast mismatch: ${JSON.stringify(result)}`);
        return passedCase("unreadableProgressWarningCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runUnreadableProgressWarningProbe(page, { disableReplaceToasts: true, seedPriorToast: true });
        ...
        return passedCase("unreadableProgressWarningFallbackWithoutReplaceToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("corruptMetricsSnapshotStatusCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ missionId, progressKey, taskKey }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, JSON.stringify({
            [missionId]: { _started: true }
          }));
        ...
        }, { missionId: ETHICS_MISSION_ID, progressKey: PROGRESS_KEY, taskKey: ETHICS_TASK_KEY });
        ...
        return passedCase("clearUnreadableTaskKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("invalidMetricsSnapshotStatusCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ progressKey }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, "{not-json");
          try {
            const managedError = (() => {
        ...
        return passedCase("corruptProgressSnapshotStatusCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey, taskName }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, "{not-json");
          const started = window.ensureMissionStarted(missionId);
          return {
        ...
        return passedCase("corruptProgressWriteGuardCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Restore rollback did not preserve original progress after silent write failure: ${JSON.stringify(result)}`);
        return passedCase("restoreSilentWriteFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearRollbackStateProbe(page);
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Clear rollback did not restore progress after partial remove failure: ${JSON.stringify(result)}`);
        ...
        return passedCase("clearRollbackCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearRollbackStateProbe(page, { silentNoOp: true });
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Clear rollback did not restore progress after silent removeItem no-op: ${JSON.stringify(result)}`);
        ...
        return passedCase("clearSilentRollbackCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("clearButtonLeavesStorageEmptyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearSuccessReplacementProbe(page);
        ...
        assert(result.clearSignalCount === 0, `Successful clear should keep its outcome toast untracked: ${JSON.stringify(result)}`);
        ...
        return passedCase("clearButtonDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearSuccessReplacementProbe(page, { disableReplaceToasts: true });
        ...
        return passedCase("clearButtonFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearEmptyReplacementProbe(page);
        ...
        return passedCase("clearButtonEmptyDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runClearEmptyReplacementProbe(page, { disableReplaceToasts: true });
        ...
        return passedCase("clearButtonEmptyFallbackWithoutReplaceToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(async ({ eventsKey, missionId, progressKey }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, JSON.stringify({
            [missionId]: { _started: true }
          }));
        ...
        return passedCase("clearRollbackDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("clearSnapshotFailureDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        const result = await page.evaluate(async ({ eventsKey, missionId, progressKey }) => {
          ...
          Storage.prototype.setItem = function (key, value) {
            if (key === progressKey && String(value).includes(missionId)) {
              return;
            }
            return originalSetItem.call(this, key, value);
          };
        ...
        return passedCase("clearRollbackFailureDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("clearResyncCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "clear", "status");
        ...
        return passedCase("clearStatusResyncCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "clear", "toast");
        ...
        return passedCase("clearToastResyncCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Restore flow did not preserve the imported progress snapshot: ${JSON.stringify(result)}`);
        ...
        return passedCase("restoreResyncCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "restore", "status");
        ...
        return passedCase("restoreStatusResyncCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "restore", "toast");
        ...
        return passedCase("restoreToastResyncCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("importSuccessDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportInputSuccessReplacementProbe(page);
        assert(result.statusTexts.join("|") === IMPORT_SUCCESS_TEXT, `Import input handler should replace prior visible toasts with its own success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("importInputDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportInputSuccessReplacementProbe(page, { disableReplaceToasts: true });
        assert(result.statusTexts.join("|") === IMPORT_SUCCESS_TEXT, `Import success fallback should replace the prior tracked toast with the imported success signal after storage restore: ${JSON.stringify(result)}`);
        ...
        return passedCase("importInputFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportCancelReplacementProbe(page);
        ...
        return passedCase("importCancelDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportCancelReplacementProbe(page, { disableReplaceToasts: true });
        ...
        return passedCase("importCancelFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportInvalidJsonReplacementProbe(page);
        ...
        return passedCase("importInvalidJsonDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportInvalidJsonReplacementProbe(page, { disableReplaceToasts: true });
        ...
        return passedCase("importInvalidJsonFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportErrorReplacementProbe(page, {
          errorText: IMPORT_EMPTY_ERROR_TEXT,
          fileBuffer: Buffer.from(JSON.stringify({ unrelated_key: true })),
          fileName: "empty-import.json",
          setupPage: async (currentPage) => {
            await currentPage.evaluate((exportedDataToastText) => {
              localStorage.clear();
              window.showToast(exportedDataToastText, "success", 3000);
            }, EXPORTED_DATA_TOAST_TEXT);
          }
        });
        ...
        return passedCase("importEmptyDropsPriorToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportErrorReplacementProbe(page, {
          errorText: IMPORT_SNAPSHOT_FAILURE_ERROR_TEXT,
          fileBuffer: Buffer.from(JSON.stringify({
            [PROGRESS_KEY]: JSON.stringify({
              "mission-ai-content": { _started: true }
            })
          })),
          fileName: "snapshot-failed-import.json",
          includeProgressRaw: true,
          setupPage: async (currentPage) => {
            await currentPage.evaluate(({ exportedDataToastText, progressKey }) => {
              localStorage.clear();
              localStorage.setItem(progressKey, "{not-json");
              window.confirm = () => true;
              window.showToast(exportedDataToastText, "success", 3000);
            }, { exportedDataToastText: EXPORTED_DATA_TOAST_TEXT, progressKey: PROGRESS_KEY });
          }
        });
        ...
        return passedCase("importSnapshotFailureDropsPriorToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportErrorReplacementProbe(page, {
          errorText: IMPORT_WRITE_FAILURE_ERROR_TEXT,
          fileBuffer: Buffer.from(JSON.stringify({
            [PROGRESS_KEY]: JSON.stringify({
              "mission-ai-content": { _started: true }
            })
          })),
          fileName: "restore-write-fail-import.json",
          includeProgressRaw: true,
          setupPage: async (currentPage) => {
            await currentPage.evaluate(({ exportedDataToastText, missionId, progressKey }) => {
              localStorage.clear();
              localStorage.setItem(progressKey, JSON.stringify({
                [missionId]: { _started: true }
              }));
            ...
            }, { exportedDataToastText: EXPORTED_DATA_TOAST_TEXT, missionId: ETHICS_MISSION_ID, progressKey: PROGRESS_KEY });
          }
        });
        ...
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Import write failure should preserve the original progress snapshot after rollback: ${JSON.stringify(result)}`);
        ...
        return passedCase("importWriteFailureDropsPriorToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runImportErrorReplacementProbe(page, {
          errorText: IMPORT_ROLLBACK_FAILURE_ERROR_TEXT,
          fileBuffer: Buffer.from(JSON.stringify({
            [PROGRESS_KEY]: JSON.stringify({
              "mission-ai-content": { _started: true }
            })
          })),
          fileName: "rollback-failed-import.json",
          includeEventsRaw: true,
          includeProgressRaw: true,
          setupPage: async (currentPage) => {
            await currentPage.evaluate(({ eventsKey, exportedDataToastText, missionId, progressKey }) => {
              localStorage.clear();
              localStorage.setItem(progressKey, JSON.stringify({
                [missionId]: { _started: true }
              }));
        ...
        return passedCase("importRollbackFailureDropsPriorToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("repairResyncCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "repair", "status");
        ...
        return passedCase("repairStatusResyncCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runStatusSignalResyncProbe(page, "repair", "toast");
        ...
        return passedCase("repairToastStatusResyncCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, taskKey, taskName }) => {
          localStorage.clear();
        ...
        return passedCase("taskDurationFallbackCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, taskName }) => {
          localStorage.clear();
          localStorage.setItem(eventsKey, JSON.stringify([
            {
          ...
          window.pmMetrics.markTaskComplete(taskName, { mission_id: missionId });
          const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
          const taskComplete = [...events].reverse().find(event => event.event_name === "task_complete" && event.mission_id === missionId);
          return {
            taskCompleteDuration: taskComplete ? taskComplete.task_duration_ms : null,
        ...
        return passedCase("outOfOrderTaskDurationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("staleMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("equalTimeMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("staleActiveMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("toolProjectStaleMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeyAliasCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeyMigrationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeySilentRemoveCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeyUnreadableCanonicalCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeyMigrationSilentWriteFailCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyTaskKeyMixedValidityCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricKeyMigrationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricProjectIdNormalizationCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricExportImportCanonicalCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartExportBackfillCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartExportLatestMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartExportRetentionCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricUnreadableAliasCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricUnreadableAliasRepairCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricUnreadableCanonicalCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricUnreadableCanonicalRepairCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricMigrationWriteFailCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricMigrationSilentWriteFailCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("legacyMetricStatsDedupeCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("invalidTaskKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("suffixedJunkTaskKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("junkMetricKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("junkMetricKeySilentRemoveCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("suffixedJunkMetricKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("suffixedMetricKeyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskName }) => {
        ...
        return passedCase("missionStartTokenCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(({ missionId, taskKey }) => {
          localStorage.clear();
        ...
        return passedCase("missionMarkerTimestampNullCase");
      } finally {
        await page.close();
      }
    }));
      ...
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey, taskName }) => {
          ...
          const taskEvents = JSON.parse(localStorage.getItem(eventsKey) || "[]")
            .filter(event => event.task_name === taskName)
        ...
        assert(!result.progressKeys.includes(ETHICS_MISSION_ID), `Mission reset did not clear mission progress state: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionResetClearsMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetReplacementProbe(page);
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Successful reset should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, MISSION_RESET_SUCCESS_TEXT) === 1, `Successful reset should record exactly one reset success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionResetDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetReplacementProbe(page, { disableReplaceToasts: true });
        assert(countExactMatches(result.statusTexts, EXPORTED_DATA_TOAST_TEXT) === 1, `Successful reset fallback should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, MISSION_RESET_SUCCESS_TEXT) === 1, `Successful reset fallback should record exactly one reset success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionResetFallbackWithoutReplaceToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionStepSuccessReplacementProbe(page);
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Step success should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(countExactMatches(result.statusTexts, MISSION_STEP_0_SUCCESS_TEXT) === 1, `Step success should record exactly one step-complete success signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionStepDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionStepSuccessReplacementProbe(page, { completeMission: true });
        assert(countExactMatches(result.statusTexts, STALE_TOAST_TEXT) === 1, `Mission completion should not duplicate the prior tracked toast on later interaction: ${JSON.stringify(result)}`);
        assert(result.statusTexts.some((text) => text.startsWith(MISSION_COMPLETION_TEXT_PREFIX)), `Mission completion should record a completion toast signal: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionCompletionDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionStepPersistenceFailureProbe(page);
        ...
        return passedCase("missionStepSaveFailureDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionStepPersistenceFailureProbe(page, { blockOnlyStepWrite: true });
        ...
        return passedCase("missionStepUpdateFailureDropsPriorToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Mission reset save failure should preserve the original mission progress after silent removeItem failure: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionResetSaveFailureDropsPriorToastsCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetRollbackFailureProbe(page);
        ...
        assert((result.progressRaw || "").includes(ETHICS_MISSION_ID), `Mission reset rollback warning case should preserve original progress after rollback: ${JSON.stringify(result)}`);
        ...
        return passedCase("missionResetRollbackDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetRollbackFailureProbe(page, { failRollbackRestore: true });
        ...
        return passedCase("missionResetRollbackFailureDropsPriorToastsCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetRollbackStateProbe(page);
        ...
        return passedCase("missionResetRollbackCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runMissionResetRollbackStateProbe(page, { silentNoOp: true });
        ...
        return passedCase("missionResetSilentRollbackCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      await page.addInitScript((eventsKey) => {
        ...
        Storage.prototype.setItem = function (key, value) {
          if (key === eventsKey && !failed && String(value).includes("\"task_complete\"")) {
            failed = true;
      ...
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey, taskName }) => {
          ...
          return {
            ...
            progressState: JSON.parse(localStorage.getItem(progressKey) || "{}")[missionId] || {}
          };
        }, {
          eventsKey: DASHBOARD_EVENTS_KEY,
          missionId: ETHICS_MISSION_ID,
        ...
        return passedCase("missionCompletionRecoveryCase");
      } finally {
        await page.close();
      }
    }));
      ...
      try {
        const result = await page.evaluate(({ eventsKey, missionId, progressKey, taskKey, taskName }) => {
          ...
          try {
            ...
            window.ensureMissionStarted(missionId);
            const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
            const taskStarts = events.filter(event => event.event_name === "task_start" && event.task_name === taskName);
            return {
              markTaskStartCalls,
        ...
        return passedCase("markerBackfillCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("markerBackfillNewerOpenCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("unreadableMissionMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runUnreadableMissionMarkerWarningProbe(page);
        assert(result.toastText === MISSION_MARKER_WARNING_TEXT, `Unreadable mission-marker warning toast mismatch: ${JSON.stringify(result)}`);
        return passedCase("unreadableMissionMarkerWarningCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runUnreadableMissionMarkerWarningProbe(page, { disableReplaceToasts: true, seedPriorToast: true });
        ...
        return passedCase("unreadableMissionMarkerWarningFallbackWithoutReplaceToastsCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(async ({ missionId, progressKey, taskKey }) => {
          localStorage.clear();
          localStorage.setItem(progressKey, JSON.stringify({
            [missionId]: { _started: true }
          }));
        ...
        assert(result.beforeClear.join("|") === MISSION_MARKER_WARNING_TEXT, `Managed storage warning should not duplicate across repeated refreshes while visible: ${JSON.stringify(result)}`);
        assert(result.afterClear.join("|") === MISSION_MARKER_WARNING_TEXT, `Managed storage warning should reappear after being dismissed while the unsafe state persists: ${JSON.stringify(result)}`);
        return passedCase("managedStorageWarningRedisplayCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("existingOpenStartCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("staleOpenStartCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("futureTaskMarkerCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("chronologicalTrimCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("oversizedMetricArrayCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("futureMetricEventCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      const pageA = await createReadyPage(context, origin);
      const sessionA = await readLatestDashboardSessionId(pageA);
      await pageA.close();

      const pageB = await createReadyPage(context, origin);
      const sessionB = await readLatestDashboardSessionId(pageB);
      await pageB.close();
      ...
      return passedCase("sessionFallbackCase");
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      return passedCase("sessionSilentWriteFailureCase");
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        ...
        return passedCase("pageViewRetryCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        ...
        return passedCase("pageViewFlushCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        ...
        return passedCase("pageViewFlushRetentionCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        ...
        return passedCase("pageViewPathRetryCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page = await createReadyPage(context, origin);
      try {
        const result = await page.evaluate(async (eventsKey) => {
        ...
        return passedCase("delegatedCtaCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("lateStatusObserverCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("toastStatusObserverCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("toastZeroMessageStickyCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("toastMessageEscapingCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("clearToastsHelperCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { errorMessage: "forced toast status write failure" }), async (page) => {
        const result = await page.evaluate(async (eventsKey) => {
          window.showToast("学习数据已导出", "success", 3000);
          await new Promise(resolve => setTimeout(resolve, 20));
          window.clearToasts?.();
        ...
        return passedCase("clearToastsPendingStatusCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "已导入 3 条学习数据", "success", "status_success_signal");
        ...
        return passedCase("toastGenericSuccessObserverCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "任务进度已重置", "warn", "status_success_signal");
        ...
        return passedCase("toastResetSuccessObserverCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "导入失败：文件不是有效的 JSON", "error", "status_error_signal");
        ...
        return passedCase("toastErrorStatusObserverCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "浏览器存储删除不完整，已保留原有数据。", "error", "status_error_signal");
        ...
        return passedCase("toastGenericErrorObserverCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "浏览器拦截了新标签页，请允许弹窗后重试", "warn", "status_error_signal");
        ...
        return passedCase("toastWarnErrorObserverCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runToastObserverProbe(page, "学习数据已清除", "warn", "status_success_signal");
        ...
        return passedCase("toastClearSuccessObserverCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statusSignalDedupeCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statusSignalResetCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("statusSignalReplaceCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { errorMessage: "forced toast status_success_signal write failure" }), async (page) => {
        const result = await page.evaluate(async (eventsKey) => {
          window.showToast("学习数据已导出", "success", 3000);
          await new Promise(resolve => setTimeout(resolve, 20));
          window.pmMetrics.track("cta_click", { control_id: "probe" });
        ...
        return passedCase("toastStatusFlushCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { trackFailures: true }), async (page) => {
        const result = await page.evaluate(async (eventsKey) => {
          const status = document.createElement("div");
          status.id = "status";
          document.body.appendChild(status);
        ...
        return passedCase("statusSignalFlushCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { trackFailures: true }), async (page) => {
        ...
        return passedCase("statusSignalFlushRetentionCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin), async (page) => {
        const result = await page.evaluate(async (eventsKey) => {
        ...
        return passedCase("statusSignalPathCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { errorMessage: "forced first queued status write failure" }), async (page) => {
        const result = await runQueuedStatusSignalProbe(page);
        ...
        return passedCase("statusSignalQueueCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithStatusSuccessWriteFailure(context, origin, { errorMessage: "forced first reset success failure" }), async (page) => {
        const result = await runQueuedStatusSignalProbe(page, { resetAfterIntermediate: true });
        assert(result.statusNames.join("|") === "导出成功|导出成功", `Queued status signal did not reset after non-signal state: ${JSON.stringify(result)}`);
        return passedCase("statusSignalQueueResetCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page1 = await createPageWithStatusSuccessWriteFailure(context, origin);
      try {
      ...
      try {
        const result = await page2.evaluate((eventsKey) => {
          const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
          return {
            statusSuccessCount: events.filter(event => event.event_name === "status_success_signal").length
          };
        ...
        return passedCase("statusSignalRetryCase");
      } finally {
        await page2.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithFirstInteractionWriteFailure(context, origin), async (page) => {
        await page.dispatchEvent("body", "pointerdown");
        await page.waitForTimeout(150);
        ...
        return passedCase("firstInteractionRetryCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithFirstInteractionWriteFailure(context, origin), async (page) => {
        ...
        return passedCase("firstInteractionFlushCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithFirstInteractionWriteFailure(context, origin), async (page) => {
        ...
        assert(result.ctaClickPath === result.laterPath, `Later tracked event did not use current page_path: ${JSON.stringify(result)}`);
        return passedCase("firstInteractionPathCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const page1 = await createPageWithFirstInteractionWriteFailure(context, origin);
      try {
      ...
      try {
        ...
        return passedCase("firstInteractionUnloadRetryCase");
      } finally {
        await page2.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const result = await runTaskStartBackfillRecoveryCase(browser, origin, "complete");
      ...
      return passedCase("taskStartBackfillCase");
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartBackfillNewerOpenCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        const result = await page.evaluate(({ eventsKey, taskName }) => {
          const taskKey = `pm_metrics_task_start_P00-dashboard::${taskName}`;
          localStorage.setItem(taskKey, String(Date.now() - 2000));
          window.pmMetrics.markTaskComplete(taskName, { done: true });
          const events = JSON.parse(localStorage.getItem(eventsKey) || "[]");
          return {
        ...
        }, { eventsKey: DASHBOARD_EVENTS_KEY, taskName: OVERVIEW_TASK_NAME });
        ...
        return passedCase("terminalFlushPendingCoreCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createPageWithPageViewWriteFailure(context, origin), async (page) => {
        const result = await page.evaluate(({ eventsKey, taskName }) => {
          ...
          return {
            count: events.length,
            eventNames: events.filter(event => event.event_name === "page_view" || event.task_name === taskName).map(event => event.event_name)
          };
        }, { eventsKey: DASHBOARD_EVENTS_KEY, taskName: OVERVIEW_TASK_NAME });
        ...
        return passedCase("terminalFlushPendingCoreRetentionCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("saveProgressSilentWriteFailureCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskBackfillRetentionProbe(page, "complete");
        ...
        return passedCase("taskStartBackfillRetentionCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const result = await runTaskBackfillAtomicityCase(browser, origin, "complete");
      ...
      return passedCase("taskStartBackfillAtomicityCase");
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskTerminalMarkerProbe(page);
        ...
        return passedCase("taskStartMarkerClearFailureCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskTerminalMarkerProbe(page, { unreadableAfterRemove: true });
        ...
        return passedCase("taskStartMarkerUnreadableAfterRemoveCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskTerminalMarkerProbe(page, { action: "error" });
        ...
        return passedCase("taskErrorMarkerClearFailureCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskTerminalMarkerProbe(page, { action: "error", unreadableAfterRemove: true });
        ...
        return passedCase("taskErrorMarkerUnreadableAfterRemoveCase");
      });
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      ...
      try {
        ...
        return passedCase("taskStartBackfillClosedCase");
      } finally {
        await page.close();
      }
    }));
    ...
    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const result = await runTaskStartBackfillRecoveryCase(browser, origin, "error");
      ...
      return passedCase("taskErrorBackfillCase");
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      return withPage(createReadyPage(context, origin), async (page) => {
        const result = await runTaskBackfillRetentionProbe(page, "error");
        ...
        return passedCase("taskErrorBackfillRetentionCase");
      });
    }));

    results.push(await runIsolatedCase(browser, server.origin, async (context, origin) => {
      const result = await runTaskBackfillAtomicityCase(browser, origin, "error");
      ...
      return passedCase("taskErrorBackfillAtomicityCase");
    }));
